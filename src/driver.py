...
def initiateNodes(filename, pubAgentAddr, sinkAgentAddr, manual):
    ...
    for h in peersHosts:
        ...
        if isLocalNode(Nodes[h]['ip']) and not manual:
            NodeProcesses.append(establishLocalNode(h, pubAgentAddr, sinkAgentAddr, neighbor))
            Nodes[h]['status'] = NodeStatus.DRIVER_INITIALIZED
...
def moveData(entranceNode, dest, data=None, dataId=None, sock=None, msgtype=None):
    ...
    netMsg = [entranceNode.encode(), packet]
...
def testBidirectionalChannel(sock, msgInQ, msgOutQ):
    ...
    for k in Nodes:
        msg = [Nodes[k]['name'].encode(), packet]
        sock.send_multipart(msg)

    while True:
        try:
            while not msgInQ.empty():
                msg = msgInQ.get(False)
                node_name = msg[0].decode() if isinstance(msg[0], bytes) else msg[0]
                Nodes[node_name]['status'] = NodeStatus.DRIVER_FUNCTIONAL
                msgInQ.task_done()
                msgOutQ.put([msg[0], b"ack"])
    ...
    try:
        ...
        for line in fp:
            ...
            for argument in line:
                if argument in ('sleep', 'set', 'reset', 'hungry', 'exit'):
...
def updateGraph(node, last, nextNode):
    ...
    if node not in graphLast:
        graphLast[node] = []

    if node not in graphNext:
        graphNext[node] = []
...
def updateDrawingGraph(node, last, nextNode):
    ...
    if last is not None and last != "None":
    ...
    if graphNetXLast.has_node(friendlySrcName):
        neighbors = list(graphNetXLast.neighbors(friendlySrcName))
        for n in neighbors:
    ...
    if graphNetXNext.has_node(friendlySrcName):
        neighbors = list(graphNetXNext.neighbors(friendlySrcName))
        for n in neighbors:
...
def verifyOperation(update, hungryNodes, graphCounter):
    ...
    if isinstance(node, bytes):
        node = node.decode()
    update_data = pickle.loads(update[1])

    if node not in NodeObjectStatus:
        NodeObjectStatus[node] = "N/A"

    print("Current Node Status(", node, ")", NodeObjectStatus[node])
    NodeObjectStatus[node] = update_data[PRStatusUpdate.STATUS]
    print("Changed Node Status(", node, ")", NodeObjectStatus[node])
    if update_data[PRStatusUpdate.SATISFY]:
    ...
    updateGraph(node, update_data[PRStatusUpdate.LAST], update_data[PRStatusUpdate.NEXT])



    updateDrawingGraph(node, update_data[PRStatusUpdate.LAST], update_data[PRStatusUpdate.NEXT])
    ...
    statusAck = PRStatusUpdate.createStatusACKMessage(update_data[PRStatusUpdate.SEQ])
    return (node.encode() if isinstance(node, str) else node, statusAck)
...
def playScenario(scenarioFile, peerSock, inQueue, outQueue, sinkServer):
    ...
    for cmd in scenario:
        ...
        if cmd['ACTION'] == 'sleep':
            ...
            dataItem = str(choice(range(1000)))
            ...
            peerSock.send_multipart([b'Set', txMsg])
            ...
            peerSock.send_multipart([cmd['ARG'].encode(), txMsg])
